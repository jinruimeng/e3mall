...
define(function(require, exports, module) {
    ...
    var cartObj = {
        ...
        optimalPromoDowngrade : function() {return ($('#isOpdg').length == 0 || $('#isOpdg').val() == "1");},
        ...
        bindCartEvent: function(){
            ...
            $('body').delegate('#container', 'click', function(event){
				...
				// 价格切换弹层
                var notPriceSwitchbox = $(tg).hasClass('plus-switch') || !$(tg).hasClass('plus-switch') && (!$(tg).parents('.plus-switch-tips').length || $(tg).parents('.price-switch-now').length > 0 || $(tg).hasClass('price-switch-now'));
        ...
        cartVenderDialogNew: function(el, recommendCombined){
        	...
        	var venderSPF = $(el) ;
        	/**
			 * 自营,FBP等续重运费凑单包邮 (选中的sku,小计,差价,总重,差重,差重信息,差重level 99:0 199:1 299:2
			 * 399:3 499:4 50kg+:5,是否只有京东自营,预留参数（FBP店铺名称）,关闭窗口回调,点击加入购物回调)
			 */
        	if (venderSPF.hasClass("shop-freight")) {
        		...
        		/**
				 * pop凑单推荐窗口，展示推荐位 pin 用户pin checkedSkuIds 选中的商品id venderId 店铺id
				 * shopId 店铺id totalPrice 店铺小计金额 freeShippingPrice pop店铺免运费最小金额
				 * freight 运费金额 function1 关闭窗口回调 function2 添加购物车回调
				 */
        ...
        getVenderInfo: function(venderIds) {
            ...
            jQuery.ajax({
                ...
                data : {t:this.t, venderIds : venderIds, random : Math.random()},
        				...
        				if(result && result.success){
        					...
        					for(var i = 0; i < len; i++) {
        						...
        						if (venderEl.length && !venderEl.hasClass("shop-freight") && !venderEl.hasClass("walmart-shop-freight")) {// 自营和fbp不处理
	        						...
	        						if(venderInfo.freightPattern == 1){ // 商家运费模式:
																		// 1. 店铺
																		// 2. 单品
																		// 3.
																		// 店铺和单品混合模式
	        							...
	        							if(venderInfo.venderFreight == 0){// 运费是
																			// 0
	        								venderEl.html('已免运费 ');
			...
			$(deliveryDiv).each(function(){
				if ($(this).attr("zy") == "1") {
					...
					if(serviceItemId != ""){
						...
						popData = popData +",venderId:"+ $(this).attr("venderId");
        ...
        updateCartInfo: function(url, param, errorMessage, callback, showSam){
            ...
            jQuery.ajax({
                ...
                dataType : "json",
                url  : url,
                data : param + "&t=" + me.t + "&outSkus=" + this.outSkus + "&random=" + Math.random() + "&locationId=" + this.ipLocation,
                success : function(result) {
            	    me.setLogin(result);
        ...
        calWeightFreight: function(){
        	...
        	$.ajax({
                ...
                type: "POST",
                dataType: "json",
                data: "t=" + this.t + "&freshGoodsInfo=" + freshGoodsInfo  + "&notFreshGoodsInfo=" + notFreshGoodsInfo + "&walmartGoodsInfo=" + walmartGoodsInfo,
                success: function(result) {
        ...
        initYanBao: function(){
            ...
            jQuery.ajax({
                ...
                success : function(result) {
					...
					// if( (!ybinfos || !ybinfos.length) && (!jshsinfos ||
					// !jshsinfos.length)){
					// return;
					// }
	                if(ybinfos){
        ...
        removeSkuDlg: function(el){
        	/*
			 * 我的关注是否降级。这块儿的处理逻辑是，通过js去获取推荐栏的[我的关注]标签。
			 * 如果该标签存在，则说明[我的关注没有降级]，删除商品的弹出信息正常显示； 如果该标签不存在，则去掉这里弹出层的关注等字样和按钮
			 * BY chenlian
			 */
        ...
        followSku: function(pids, objel, bremoved, params, selectstate, callback, removegiftboxids, onlyFollow){
            ...
            $.getJSON("//follow-soa.jd.com/rpc/product/batchfollow?sysName=cart.jd.com&productIds=" + pids + "&random=" + Math.random() + "&callback=?", function(result) {
                    if(result.success){
        ...
        loadCoupons: function(venderId){
        	...
        	try {
        		...
        		var getDisCouponInfo = function (disList, high) {
        			var result = [];
        			if (disList.length == 1) {
        				result.push(disList[0].discount * 10 + '折');
        				result.push('满' + disList[0].quota + '享' + result[0]);
        				result.push(result[1] + '，最多减¥' + high);
					} else {
						result.push(disList[disList.length-1].discount * 10+'~'+disList[0].discount * 10+'折');
						result.push('满' + disList[0].quota + '享' + disList[0].discount * 10 + '折');
						var hoverDesc = '';
    							...
    							for(var j = 0; j < skus.length; j++){
    								if(!combiedStr && itemsSelectedIds){
    									if(zycoupon.couponKind != 0 && itemsSelectedIds.indexOf(skus[j]) >= 0 && !me.isOverseasLoc() && me.t != me.myfridge ){ // 屏蔽已领取全品券
    										combiedStr = '<span class="cextra ml20"><a target="_blank" href="//search.jd.com/Search?coupon_batch='+zycoupon.batchId+'">去凑单</a><i></i></span>';
    									}
    								}
    								var _imgsrc = null;
    								...
    								'<div class="p-coupon-item' + (zycoupon.taked ? ' p-coupon-item-gray">' : '">') +
    									...
    									'<div class="coupon-msg">' + 
    										'<div>' +
    											...
    											(zycoupon.disCoupon ? ('<span class="cinfo" title="' + disCouponInfo[2] + '"> ' + disCouponInfo[1] + '</span>') : ('<span class="cinfo"> '+((zycoupon.quota) ? '满¥'+(zycoupon.quota) : '')+'减¥'+(zycoupon.discount)+'</span>')) + 
    											combiedStr +
    										'</div>' + 
    										(zycoupon.overlap == 2 ? '<span class="coverlay fl" title="' + zycoupon.overlapDesc + '">可叠加</span>' : '') +
    						...
    						for(var i=0; i<couponSets.length; i++){
    							...
    							couponHtml += 
    								'<div class="p-coupon-item' + (coupon.taked ? ' p-coupon-item-gray' : '')+'">'+
    									...
    									'</div>'+
    									'<div class="coupon-msg">'+
    										(coupon.disCoupon ? ('<div title="' + disCouponInfo[2] + '">'+type+','+ disCouponInfo[1] +'</div>') : ('<div>'+type+',满¥'+ quota +'减¥'+discount+'</div>')) + 
    										(coupon.overlap == 2 ? '<span class="coverlay fl" title="' + coupon.overlapDesc + '">可叠加</span>' : '') +
                    ...
                    jQuery.ajax({
                        ...
                        success: function (data) {
                            if(data && data.success && data.result && data.result.packing) {
                                ...
                                giftcartHtml.push('    <div class="gcb-title"><i class="jd-giftcard-icon"></i><span>选包装</span></div>');
...
// giftcartHtml.push(' <span class="gcb-cardtips">附赠精美贺卡</span>');
        ...
        initPlusPriceSwitchEvent: function() {
            ...
            $('.cart-warp').delegate('.select-priceswitch', 'click', function(event){
                ...
                var el = $(this);
                var pnode = el.closest('.plus-switch-tips');
                var data = $('input:radio:checked', pnode).val();
...
// + "&promoID=" + curPromoId
            ...
            $('.cart-warp').delegate('.cancel-priceswitch', 'click', function(event){
                $(this).closest('.plus-switch-tips').slideUp(200);
                if(me.isLowIE()){
                    $(this).closest('.item-item').parent().css('z-index', 'auto');
                    $(this).closest('.item-item').css('z-index', 'auto');
                }
        ...
        clearDialog: function(){
			...
			$('.plus-switch-tips').each(function(){
                $(this).slideUp(200);
            });
        ...
        initMultiGiftPoolEvent: function(){
            ...
            $('.cart-warp').delegate('div.gift-3c-main-expan div.btn-1', 'click', function(e){
                ...
                // TODO 校验是否重复
// if ($(this).attr('select_ids') == giftids) {
// $('.gift-3c-title, .gift-3c-main',
// $(this).closest('.item-item')).addClass('hide');
// return;
// }
                var act = '/changePoolGifts.action';
                var params = "gids=" + giftids
        ...
        getOptimalPromo : function () {
        	try {
	        	...
	        	$("[calop]").each(function (){
	        		...
	        		",cid:"+ $(this).attr("cid") + ", venderId:" + $(this).attr("venderid") +// ",
																								// shopId:"
																								// +
																								// $(this).attr("shopid")
																								// +
	            ...
	            jQuery.ajax({
	                url: me.iurl + "/getOptimalPromo.action",
	                type : "POST",
	                dataType : "json",
	                data: {skus : "["+skus+"]", fictVenderIds: $("#fictVenderIds").val()},
	                success: function (result) {
	                    if (result && result.promoPrice && result.promoRela) {
	                    	...
	                    	if (balance >= 1) {
	                    		var param = "";
		                    	for (i in result.promoRela) {
		                    		param += i + "_" + result.promoRela[i] + ",";
		                    	}
		                    	if (param.length > 0) {
        ...
        initcarService:function(){
        	...
        	if(serviceInfoIds != ""){
        		jQuery.ajax({
        			...
        			data : "serviceInfoIds=" + serviceInfoIds,
        			success : function(result) {
        				...
        				if (result && result.length > 0) {
        					...
        					for(var i = 0; i < len; i++){
        						...
        						for(var j = 0; j < length; j++){
        							...
        							if(carm.length == 0){
        								$('div[id='+skuId+']').find('.p-props-new').append('<div class="props-txt" carmodel title="'+car.nameCn+'">车型：'+car.nameCn+'</div>')
        							}
        						}
        					}
        				}
    ...
    $(function(){
        // global(); // 切换到2.0版本初始化
        // if(isHK){
        // require.async("jdf/1.0.0/unit/globalInit/1.0.0/globalInit",
		// function(global) {});
